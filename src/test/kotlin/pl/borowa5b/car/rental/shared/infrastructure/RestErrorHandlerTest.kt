...
class RestErrorHandlerTest {
    ...
    fun `should handle validation error exception`() {
        ...
        val fieldName = "carId"
        val exception = ValidationErrorException(ValidationError(title, message, fieldName))
        val restErrorHandler = RestErrorHandler()
        ...
        assertThat(problem.title).isEqualTo(title)
        assertThat(problem.detail).isEqualTo(message)
        assertThat(problem.parameters).containsEntry("fieldName", fieldName)
    }
    ...
    fun `should handle validation exception`() {
        ...
        val validationError1 = ValidationError(
            "Field has invalid value",
            "Field `carId` must start with `CAR`",
            "carId"
        )
        val validationError2 = ValidationError(
            "Field has invalid value",
            "Field `customerId` must start with `CTR`",
            "customerId"
        )
